...
def assert_local_model_controls(page) -> None:
    ...
    expect(coder.get_by_role("button", name="INITIATE", exact=True)).to_be_enabled()
    expect(coder.get_by_role("button", name="TERMINATE", exact=True)).to_have_count(0)

    coder.get_by_role("button", name="INITIATE", exact=True).click()
    expect(coder.locator(".model-state")).to_have_text("ONLINE")
    expect(model_35b.get_by_role("button", name="INITIATE", exact=True)).to_be_disabled()
    expect(coder.get_by_role("button", name="INITIATE", exact=True)).to_have_count(0)
    expect(coder_actions).to_have_count(1)
    expect(coder_actions).to_have_class(re.compile(r"\bterminate\b"))
    expect(coder.get_by_role("button", name="TERMINATE", exact=True)).to_be_enabled()
    OUTPUT_DIRECTORY.mkdir(exist_ok=True)
    page.screenshot(
    ...
    coder.get_by_role("button", name="TERMINATE", exact=True).click()
    expect(coder.locator(".model-state")).to_have_text("OFFLINE")
    expect(coder.get_by_role("button", name="TERMINATE", exact=True)).to_have_count(0)
    expect(coder_actions).to_have_class(re.compile(r"\binitiate\b"))
    expect(model_35b.get_by_role("button", name="INITIATE", exact=True)).to_be_enabled()
